...
# Calculate Max Pain with LTP
def calculate_max_pain(call_df, put_df):
    strike_prices = call_df['Strike']
    total_loss = []
    for strike in strike_prices:
        call_itm = call_df[call_df['Strike'] < strike]
        if not call_itm.empty:
            call_loss = ((strike - call_itm['Strike']) * call_itm['OI'] - call_itm['LTP'] * call_itm['OI']).sum()
        else:
            call_loss = 0
        
        put_itm = put_df[put_df['Strike'] > strike]
        if not put_itm.empty:
            put_loss = ((put_itm['Strike'] - strike) * put_itm['OI'] - put_itm['LTP'] * put_itm['OI']).sum()
        else:
            put_loss = 0
        
        total_loss.append(max(call_loss + put_loss, 0))
...
def main():
    ...
    with st.sidebar:
        ...
        sold_strike = st.number_input("Sold Call Strike:", value=840.0)
        sold_premium = st.number_input("Sold Premium:", value=8.0)
        lot_size = st.number_input("Lot Size:", value=1000)
        ...
        if data and 'records' in data and 'expiryDates' in data['records']:
            ...
            if selected_expiry:
                ...
                if call_df is not None and put_df is not None:
                    ...
                    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Data", "OI Analysis", "Volume Analysis", "Price Analysis", "Advanced Tools"])
                    ...
                    with tab5:
                        ...
                        heatmap_data = pd.DataFrame({
                            'Strike': call_df['Strike'],
                            'Call_OI': call_df['OI'],
                            'Put_OI': put_df['OI']
                        })
                        fig_heatmap = go.Figure(data=go.Heatmap(
                            z=[heatmap_data['Call_OI'], heatmap_data['Put_OI']],
                            x=heatmap_data['Strike'],
                            y=['Call OI', 'Put OI'],
                            colorscale='Viridis'
                        ))
                        fig_heatmap.update_layout(title=f'OI Distribution Heatmap (Expiry: {selected_expiry})')
                        st.plotly_chart(fig_heatmap, use_container_width=True)
                        
                        # P&L Simulator
                        st.subheader("P&L Simulator for Sold Call")
                        spot_range = range(int(min(call_df['Strike'])), int(max(call_df['Strike'])) + 1, 10)
                        pl_values = [max((spot - sold_strike), 0) * lot_size - sold_premium * lot_size for spot in spot_range]
                        fig_pl = px.line(
                            x=spot_range,
                            y=pl_values,
                            labels={'x': 'Spot Price at Expiry', 'y': 'Profit/Loss (₹)'},
                            title=f'P&L for Sold {sold_strike} Call (Premium: {sold_premium})'
                        )
                        fig_pl.add_vline(x=underlying_value, line_dash="dash", line_color="blue", annotation_text="Current Spot")
