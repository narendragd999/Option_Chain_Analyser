...
def get_alert_template(recommendation: Dict, ticker: str, expiry: str) -> str:
    ...
    template = (
        "*SMART SELL CALL ALERT*\n"
        f"Stock: *{ticker}*\n"
        f"Strike: *{recommendation['Strike']:.2f}*\n"
        f"Expiry: *{expiry}*\n"
        f"Premium: *₹{recommendation['Premium']:.2f}*\n"
        f"Risk/Reward: *{recommendation['Risk_Reward']:.2f}*\n"
        f"Distance from Resistance: *{recommendation['Distance_from_Resistance']:.2f}*\n"
        f"Reason: *{recommendation['Reason']}*"
    )
    return template

# Utility Functions (unchanged except for check_alerts)
...
def check_alerts(alerts: List[Dict], current_ticker: str, underlying: float, call_df: pd.DataFrame, put_df: pd.DataFrame, 
                 sold_strike: Optional[float], pcr: float, resistance_strike: float) -> Tuple[List[str], List[int]]:
    """Updated alert checking with additional condition for resistance proximity."""
    triggered_alerts = []
    alerts_to_remove = []
    ...
    for i, alert in enumerate(alerts):
        ...
        # New Alert Type: Resistance Proximity
        elif alert_type == "Resistance Proximity" and resistance_strike is not None:
            proximity_threshold = threshold  # e.g., 1% of resistance_strike
            if abs(underlying - resistance_strike) / resistance_strike * 100 <= proximity_threshold:
                triggered_alerts.append(f"Resistance Proximity Alert for {current_ticker}: Underlying ({underlying:.2f}) near Resistance ({resistance_strike:.2f})")
                triggered = True
...
def main():
    ...
    # Ensure alerts persist across refreshes
    if 'alerts' not in st.session_state:
        st.session_state['alerts'] = load_alerts()    
    ...
    with st.sidebar:
        ...
        enable_telegram_alerts = st.checkbox("Enable Telegram Alerts for Smart Trade Suggestions", 
                                       value=st.session_state.get('enable_telegram_alerts', False))

        # Persist Telegram settings in session state
        st.session_state['telegram_bot_token'] = telegram_bot_token
        st.session_state['telegram_chat_id'] = telegram_chat_id
        st.session_state['enable_telegram_alerts'] = enable_telegram_alerts

        st.subheader("Alerts")
        alert_type = st.selectbox("Alert Type:", ["Spot Price", "OI Change", "PCR", "Resistance Proximity"])
        alert_threshold = st.number_input("Threshold:", value=0.0, step=0.1)
        alert_direction = st.selectbox("Direction:", ["Above", "Below"]) if alert_type != "Resistance Proximity" else None
        is_one_time = st.checkbox("One-Time Alert", value=False)
        if st.button("Add Alert"):
            new_alert = {
                "ticker": ticker,
                "type": alert_type,
                "threshold": alert_threshold,
                "direction": alert_direction if alert_type != "Resistance Proximity" else None,
    ...
    if alerts:
        for i, alert in enumerate(alerts):
            if alert.get('ticker') == ticker:
                alert_str = f"{alert['type']} - {alert['threshold']} ({alert.get('direction', 'N/A')})"
                if st.checkbox(alert_str, key=f"alert_{i}", value=True):
                    continue  # Keep alert if checked
                else:
                    alerts.pop(i)
                    save_alerts(alerts)
                    st.session_state['alerts'] = alerts
                    st.success(f"Alert '{alert_str}' removed!")
                    break  # Exit loop after modification to avoid index issues
    else:
        st.info("No alerts set for this ticker.")
    ...
    triggered_alerts, alerts_to_remove = check_alerts(st.session_state['alerts'], ticker, underlying, call_df, put_df, 
                                                    st.session_state['sold_strike'], pcr, resistance_strike)
    if alerts_to_remove:
    ...
    with tabs[7]:
        ...
        if recommendations[0]["Strike"] is None:
            ...
            # Send Telegram Alert for Top Pick with Specific Conditions
            if (top_pick and enable_telegram_alerts and telegram_bot_token and telegram_chat_id):
                distance_condition = top_pick['Distance_from_Resistance'] == 0
                resistance_proximity = resistance_strike and abs(underlying - resistance_strike) / resistance_strike * 100 <= 1  # Within 1% of resistance
                if (distance_condition or resistance_proximity) and ('last_top_pick' not in st.session_state or st.session_state['last_top_pick'] != top_pick):
                    ...
                    st.success(f"Smart Telegram alert sent for top pick! (Distance: {top_pick['Distance_from_Resistance']:.2f}, Underlying near Resistance: {resistance_proximity})")
